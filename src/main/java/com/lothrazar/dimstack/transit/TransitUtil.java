		...
		for (int x = -2; x <= 2; x += 4) {
			for (int z = -2; z <= 2; z += 4) {
				for (int y = !tile.goesUpwards() ? 0 : -4; y <= (!tile.goesUpwards() ? 4 : 0); y++)
					world.setBlockState(pos.add(x, y, z), Blocks.STONEBRICK.getDefaultState(), 2);
			}
		}
		...
		for (int x = -2; x <= 2; x++) {
			for (int z = -2; z <= 2; z++) {
				for (int y = !tile.goesUpwards() ? 0 : -4; y <= (!tile.goesUpwards() ? 4 : 0); y += 4)
					if (x != 0 || y != 0 || z != 0) world.setBlockState(pos.add(x, y, z), Blocks.STONEBRICK.getDefaultState(), 2);
			}
		}
